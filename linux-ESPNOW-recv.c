...
int create_raw_socket(char *dev, struct sock_fprog *bpf)
{
	...
	uint8_t data[48] = {
		0x00, 0x00, 0x26, 0x00, 0x6f, 0x08, 0x00, 0x00, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa,
		0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0x12, 0x01, 0x6c, 0x09, 0x80, 0x04, 0xcc, 0xcc,
		0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xd4, 0x00, 0x33, 0x33, 0x58, 0xbf, 0x25, 0x82, 0x8e, 0xd8
	};
	
  //Transmitted packet looks like this 
//0000   00 00 19 00 6f 08 00 00 1e 23 24 0c 00 00 00 00
//0010   12 0c 6c 09 80 04 c5 af 00 d4 00 3c 00 58 bf 25
//0020   82 8e d8 e9 6b 0d 4d  
//39 bytes instead of 48 bytes. data [2,3] = 0x26 00 corresponds to header length 38 bytes. -gets altered to 0x19 00 (25 bytes). 
//data [4,5,6,7] correspond to Present Flags in rtap header. Also gets altered. 
//data [8,9,10,11,12,13,14,15] = 8 bytes = MAC timestamp - also altered.
//data [16-23] = 8 bytes discarded.
//data [24] cap[16] - Flags. 0x10 gets changed to 0x12
//data [25] cap [17]- Data rate 0x0C = 6 mbps - unchanged
//data [26,27] cap [18-19]- Channel frequency 0x6c 0x09 = 2412 MHz - unchanged
//data [28,29] cap [20-21] - 0x80 0x04 Channel flags Altered
//data [?30-37] cap [22] 0xc5 = Antenna Signal, Cap [23] 0xaf = Antenna Noise, Cap [24] 00 = Antenna number 0.
//data [38-39] cap [25-26] = 0xd4 00 - Frame control field  - unchanged
//data [40-41] cap [27-28] = Duration (must be 00 00 for ACK frames)  - Altred.
//data [42-47] cap [29-34] = Receiver Address = unchanged.
//cap [35-38] = Four bytes of Frame Check Sequence.


//Working ACK frame looks like this 
//0000   00 00 19 00 6f 08 00 00 7b e4 ad 0a 00 00 00 00
//0010   10 02 6c 09 80 04 b1 ab 00 d4 00 00 00 58 bf 25
//0020   82 8e d8 db 2d 6f 9f
