...
exec >checkandsendlogfile.txt 2>&1
...
#  exit 0
mylocationdir="/home/***"
myFILE1=$mylocationdir"sensordata"$(date -d "yesterday 13:00" '+%Y-%m-%d')
myFILE2=$mylocationdir"errorlog"$(date -d "yesterday 13:00" '+%Y-%m-%d')
if test -f "$myFILE1"; then
	echo "Sensordata" | heirloom-mailx -a "$myFILE1" -s "Sensordata" ***@gmail.com
fi
if test -f "$myFILE2"; then
	echo "Errorlog" | heirloom-mailx -a "$myFILE2" -s "Errorlog" ***@gmail.com 
#heirloom-mailx is basically s-nail
fi
}
