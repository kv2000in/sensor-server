...
int main(int argc, char **argv)
{
	if (argc < 3)
	{
		fprintf(stderr, "Usage: %s <interface> <hex_byte>\n", argv[0]);
		return EXIT_FAILURE;
	}
	
	char *dev = argv[1];
	char *hex_byte_str = argv[2];
	uint8_t additional_byte = (uint8_t)strtol(hex_byte_str, NULL, 16);
...
LABEL_CLEAN_EXIT:
	...
	printf("***** Packet sent with additional byte: 0x%02x *****\n", additional_byte);
